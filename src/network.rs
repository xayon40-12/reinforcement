...
impl Activations {
    fn apply(&self, input: Float) -> Float {
        ...
        match self {
            Id => input,
            Relu => input.max(0.0),
            Sigmoid => 2.0 * (1.0 + (-input).exp()).recip() - 1.0,
        }
    }
...
impl<const NI: usize, const NO: usize> Default for Layer<NI, NO> {
    fn default() -> Self {
        Self {
            weights: [([1e-1; NI], Activations::default()); NO], // WARNING: this 1e-3 should be a random number
            gradient: [[0.0; NI]; NO],
            activations: [0.0; NO],
            inputs: [0.0; NI],
        }
...
impl<const NI: usize, const NO: usize> ForwardNetwork<NI, NO> for Layer<NI, NO> {
    fn forward(&mut self, input: [Float; NI]) -> [Float; NO] {
        self.inputs = input;
        std::array::from_fn(|o| {
...
impl<const NI: usize, const NH: usize, const NO: usize, O: Network<NH, NO>> ForwardNetwork<NI, NO>
    for Layers<NI, NH, O>
{
    fn forward(&mut self, input: [Float; NI]) -> [Float; NO] {
...
impl<const NI: usize, const NO: usize, N: Network<NI, NO>> Reinforcement<NI, NO, N> {
    pub fn reinforce(
        &mut self,
        r: Float,
        emphasis: Float, // must be positive, and should me relatively small
    ...
    for Reinforcement<NI, NO, N>
{
    fn forward(&mut self, input: [Float; NI]) -> [Float; NO] {
        ...
        self.network
            .update_gradient(self.r, output.map(|v| v * self.emphasis));
        output
    }
}
